...
This repo instruments **AWS Budgets** across every environment. It is built
from the `Terraform-AWS-Template` scaffold — same Terragrunt + AWS + GitHub
Actions layout, same CI contract, same rules.

- One AWS Budget per environment (`dev`, `qa`, `prod`), monthly cost-based.
- Two email notifications per budget, driven by AWS Budgets natively (no SNS
  in front):
  - **ACTUAL >= `actual_threshold_percent` of monthly limit** — "you have
    breached the budget".
  - **FORECASTED >= `forecasted_threshold_percent` of monthly limit** — "you
    are on track to breach the budget this month".
- Per-env monthly amount (USD) and recipient email list live in the env's
  `terragrunt.hcl` `inputs = {}` block. Thresholds have provider-independent
  defaults in the module (`100%` for both) but can be overridden per env.

Repo layout (identical to the template):

- `terraform/terragrunt.hcl` — shared root config: providers, remote state,
  default tags, common locals, and the shared `inputs = {}` that propagates
  `project` / `environment` to the module. Change here = affects **every**
  environment.
- `terraform/environments/<env>/` — one folder per environment
  (`dev`, `qa`, `prod`). Each contains:
  - `env.hcl` — per-env locals (`environment`).
  - `terragrunt.hcl` — includes the root config and sets per-env `inputs`
    (`monthly_budget_usd`, `notification_email_addresses`, optionally
    threshold overrides).
- `terraform/modules/budget/` — the single reusable module used by every env.
  A change here re-plans / re-applies **every** environment.
- `.github/workflows/terraform-plan.yml`, `terraform-apply.yml`, `trivy.yml`
  — same CI contract as the template.
...
- **AWS Budgets is a global (account-level) service.** The Terraform provider
  still needs a region (any region works). Do not add a second provider or a
  separate `us-east-1` alias just for Budgets — the root provider is fine.
- **The S3 state bucket must pre-exist** in each AWS account before the first
  ...
  the same state.
- **Email addresses are surfaced in AWS Budgets' console** for anyone with
  `budgets:ViewBudget` in the account. Don't put personal addresses in here —
...
- **Module layout** (per Google general-style guide): every module has
  ...
  by purpose into extra files (`notifications.tf`, `iam.tf`) — do not give
  every resource its own file.
- **Variables**: always typed, always described. Numeric variables carry
  units in the name (`monthly_budget_usd`, `actual_threshold_percent`).
  Booleans are named positively (`enable_x`, not `disable_x`). No default
  for env-specific values (budget amount, email list); defaults are fine for
  env-independent values (threshold percentages).
- **Outputs**: always described. Reference resource attributes
  (`aws_budgets_budget.main.arn`), never pass through an input variable — the
...
- **Resource identifiers**: snake_case. If a module has exactly one resource
  ...
  name.
- **Stateful resources** — AWS Budgets are not stateful data stores, so
  `prevent_destroy` is not required here. A destroyed budget simply stops
  alerting; recreate to restore.
- **Provider pinning**: in the root-generated `versions.tf`, pin providers to
  a **minor** version (`~> 5.42.0`), not a major (`~> 5.0`). Patch bumps are
  automatic; minor bumps are a deliberate PR.
...
- Plan roles: read-only + state bucket write. Apply roles: only the write
  actions the modules actually use — for this repo, `budgets:*` on the
  budget name, plus the state bucket. No `*:*` on real accounts.
...
- If a design choice has real tradeoffs (e.g., "one org-wide budget or one
  per-env budget?"), name the tradeoff in terms of a Well-Architected pillar
  or SRE concept, give a recommendation, and move on unless the user pushes
  back.
