...
terraform {
  source = "${get_parent_terragrunt_dir()}/modules/budget"
}
...
# Inputs common to every environment. Per-env overrides (amount, emails,
# optional threshold overrides) go in each env's terragrunt.hcl.
inputs = {
  project     = local.project
  environment = local.environment
}
